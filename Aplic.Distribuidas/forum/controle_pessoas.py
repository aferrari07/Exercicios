    ...
    for disciplina in disciplinas:
        if (id_disciplina == disciplina['id_disciplina']):
            if id_aluno in disciplina['alunos']:
                return jsonify({'response':'True', 'aluno':'True'}), 200
            else:
                return jsonify({'response':'True', 'aluno':'False'}), 200
    return jsonify({'response':'False', 'error':'disciplina nao encontrada'}),404

@app.route('/participa/<int:ra>/<int:id_disciplina>/', methods=['GET'])
def participa(ra,id_disciplina):
    lista = []
    for disciplina in disciplinas:
        if (id_disciplina == disciplina['id_disciplina']):
            if ra in disciplina['alunos']:
                bypass = disciplina['alunos']
                for scan in alunos:
                    if scan['id_aluno'] in bypass:
                        lista.append(scan['nome'])
            if ra in disciplina['professores']:
                bypass1 = disciplina['professores']
                for scan1 in professores:
                    if scan1['id_professor'] in bypass1:
                        lista.append(scan1['nome'])
            return jsonify({'response':'True','participantes':lista}), 200
    return jsonify({'response':'False', 'error':'disciplina nao encontrada'}),404
