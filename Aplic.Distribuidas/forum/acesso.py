...
def participantes(ra, id_disciplina):
    r = requests.get('http://localhost:5000/participa/'+str(ra)+'/'+str(id_disciplina)+'/')
    if 'participantes' in r.json():
        return r.json()['participantes'] == 'True'
    return False, 'inexistente'
'''
Agora, de runtests.
